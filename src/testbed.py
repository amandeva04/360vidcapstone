import numpy as np
import pandas as pd
from src.model import SequenceLSTM
from src.utils import angular_distance_rad
from src.data_loader import HeadGazeDataset
import torch
...
class ViewportPredictor:
    ...
    def predict(self, trace_window):
        # trace_window is a small df of recent head movement
        # Convert to input tensor (T,7)
        if len(trace_window) < 5:
            return {"yaw": 0, "pitch": 0, "roll": 0}
        X = trace_window[['qw','qx','qy','qz','x','y','z']].values.astype(np.float32)
        X = torch.from_numpy(X[None]).to(self.device)
        with torch.no_grad():
            pred = self.model(X).cpu().numpy().squeeze()
        # Convert predicted gaze vector to approximate yaw/pitch
        yaw = np.degrees(np.arctan2(pred[0], pred[2]))
        pitch = np.degrees(np.arcsin(pred[1]))
...
class Experiment:
    ...
    def run(self):
        ...
        for i, row in traces.iterrows():
            ...
            pred_viewport = self.predictor.predict(traces.iloc[max(0, i-29):i+1])
...
if __name__ == "__main__":
    dataset = DatasetLoader("video.mp4", "data/videoMeta.csv")
    predictor = ViewportPredictor("outputs/best_model.pt")
    simulator = StreamingSimulator()
    evaluator = QoEEvaluator()

    exp = Experiment(dataset, predictor, simulator, evaluator)
    exp.run()
